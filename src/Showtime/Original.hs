...
--        hvae to bump the clock by some amount first.
--
...
openResource (TID me) s0@(State tv lg) =
    let myt0 = myTime (TID me) s0
        _myt1 = myt0 + openTimeDelta
        -- Register our join attempt in the log:
        lg2 = S.insert (myt0,TID me) lg
        s1 = tickN openTimeDelta (TID me) (State tv lg2) in
    trace (show (TID me)++ ": openResource: new log, new state: " ++show (lg2, s1)) $ do
     t1 <- getNextShow (TID me) s1
     -- case readParticipants t1 s1 of
     --   Nothing -> trace (show (TID me)++": openResource: other clocks not up to showtime "
     --                      ++show t1++" in state:\n  "++show s1)
     --              Nothing
     --   Just _ ->
     let myJoinTime = t1 + myEpsilon (TID me) in
        trace (show (TID me)++ ": openResource: Computed next show "++show t1
               ++" with my offset: "++show myJoinTime) $
          if t1 > myt0
          then return (tickN (myJoinTime - myt0) (TID me) s1, t1)
          else return (s1, t1)
...
getNextShow myi s0@(State tv lg) =
    trace (show myi ++ ": getting next show at time "++show myt
           ++" from times "++show times++" in state "++show s0++" result = "++show result) $
    result
 where
...
interp2 :: Prog -> (Set (State,Oplog), Set (State,Prog,Oplog))
interp2 ops =
 ...
 where
  ...
  -- Searh K levels out in the graph:
  ...
  expand (s0,ps0,ol) =
    ...
    [ -- TODO: This case is partial!
      case L.splitAt thrd ps0 of
       -- The thread is tapped out of ops.  Bump its clock to show it won't be doing anything else:
       (frnt, (myid, []) : bk) ->
            ...
            BlockedOpen showStrt ->
                ...
                case readParticipants showStrt s0 of
                  Nothing -> trace (show myid++": expand: other clocks not up to showtime "
                                   ++show showStrt++" in state:\n  "++show s0++" oplog "++show ol)
                             Nothing
                  Just locals ->
                      -- Her's the MAGIC, we don't need to wait for
...
threadOrder p =
   case summarize (interp2 p) of
     ...
     (Just ol,_) ->
       L.map (\(OpInst (TID i) _ _) -> i) (Fld.toList ol)
